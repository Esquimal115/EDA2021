...
public class BuscadorDeLaBibl {
    ...
    // UN Map index, que representa el Indice de una BD. La clase de sus 
    // Claves es Termino y la de sus Valores ListaConPI<Posting>
    ...
    public ListaConPI<Termino> hapax() {
        ...
        ListaConPI <Termino> keys = index.claves();
        keys.inicio();

       while (!keys.esFin()){

           Termino aux = new Termino(keys.recuperar().toString());
           String term = aux.termino.toLowerCase();
           term = term.split(" ")[0];

           ListaConPI<BuscadorDeLaBibl.Posting> valor = index.recuperar(new Termino(term));
