...
public class TestTablaHash {
    ...
    public static void main(String[] args) {
        ...
        if (!rehash) {
            ...
            System.out.println("... La constante TablaHash.REHASHING, vale true??\n    !!Compruebalo antes de modificar tu codigo!!");
        }
    ...
    private static boolean testRehashing() {
        ...
        int nrH1 = 1 + (int)((double)t1 * 0.75D);

        int t2;
        for(t2 = 0; t2 < nrH1 - 1; ++t2) {
            t.insertar(new Integer(t2), 0);
        }
        ...
        if (t.talla() != nrH1) {
            ...
            if (t2 != t2Ref) {
                ...
                for(int i = 0; i < nrH1 - 1 && ok; ++i) {
                    Integer oi = (Integer)t.recuperar(new Integer(i));
                    if (oi == null) {
                        ok = false;
                ...
                if (ok) {
                    Integer oi = (Integer)t.recuperar(new Integer(29));
                    if (oi == null) {
                        ok = false;
                        res = res + "\n\tNo se han copiado todos los datos";
                    }
    ...
    private static boolean testCosteMLocalizar() {
        ...
        if (res) {
            ...
            for(int i = 0; i < 10; ++i) {
                thI.insertar(new Integer(i * 53), i);
            }
    ...
    private static final double miCosteMLocalizar(TablaHash t) {
        int colisiones = 0;

        for(int i = 0; i < t.elArray.length; ++i) {
            colisiones += t.elArray[i].talla() * (1 + t.elArray[i].talla()) / 2;
        }

        return (double)colisiones / (double)t.talla;
    }
}