...
public class GUIAbb extends JFrame implements ActionListener {
    ...
    private static <E> int altura(NodoABB<E> actual) {
        return actual == null ? 0 : 1 + Math.max(altura(actual.izq), altura(actual.der));
    }
    ...
    private void calculateLocations() {
        ...
        if (root != null) {
            ...
            this.calculateLocation(root, 2147483647, 2147483647, 0);
        }
    ...
    private void calculateLocation(NodoABB<Integer> n, int left, int right, int top) {
        if (n != null) {
            ...
            this.calculateLocation(n.izq, 2147483647, center - this.child2child / 2, top + 2 * this.fm.getHeight() + this.parent2child);
            this.calculateLocation(n.der, center + this.child2child / 2, 2147483647, top + 2 * this.fm.getHeight() + this.parent2child);
        }
    }
    ...
    public void paint(Graphics g) {
        ...
        g.drawString(String.format("Altura óptima = %d", aO), 10, 60);
        if (aO != aR) {
        ...
        this.drawTree(g2d, this.tree.raiz, 2147483647, 2147483647, this.fm.getLeading() + this.fm.getAscent());
        this.fm = null;
    }
    ...
    private int alturaOptima(int t) {
        return t == 0 ? 0 : (int)Math.ceil(Math.log((double)(t + 1)) / Math.log(2.0D));
    }
}